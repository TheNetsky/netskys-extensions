...
var source = (() => {
  ...
  __export(main_exports, {
    Mitaku: () => Mitaku,
    MitakuExtension: () => MitakuExtension
  });
  init_buffer();
  ...
  var pbconfig_default = {
    ...
    badges: [],
    capabilities: import_types2.SourceIntents.MANGA_CHAPTERS | import_types2.SourceIntents.DISCOVER_SECIONS | import_types2.SourceIntents.SETTINGS_UI | import_types2.SourceIntents.MANGA_SEARCH | import_types2.SourceIntents.CLOUDFLARE_BYPASS_REQUIRED,
    developers: [
      {
        name: "Netsky",
  ...
  // src/Mitaku/parser.ts
  init_buffer();
  var Parser3 = class {
    async parseMangaDetails($2, mangaId, source) {
      const images = this.parseImages($2);
      const title = Application.decodeHTMLEntities($2(".entry-title").first().text().trim());
      const artist = Application.decodeHTMLEntities($2("p:contains(Cosplayer:)").text().trim().replace("Cosplayer:", "").trim());
...
Images: ${images.length}`;
      ...
      const tagGroups = [{ title: "genres", id: "genres", tags: genres }];
      return {
        mangaId,
        mangaInfo: {
          shareUrl,
    ...
    parseChapterList(sourceManga, source) {
      return [{
        sourceManga,
    ...
    async parseChapterDetails($2, chapter, source) {
      const pages = this.parseImages($2);
      return {
  ...
  var Mitaku = new MitakuExtension();
