...
import { MangaHasu } from '../MangaHasu/MangaHasu'
import chai from 'chai'
import chaiAsPromised from 'chai-as-promised'

describe('MangaHasu Tests', () => {

    const wrapper: APIWrapper = new APIWrapper()
    const source: Source = new MangaHasu(cheerio)
   ...
   */
    const mangaId = '19-days-oor-p137.html' // 19-days-oor-p137.html
    ...
    it('Testing home page results for recommend titles', async () => {
        const results = await wrapper.getViewMoreItems(source, 'recommend', {}, 1)

        expect(results, 'This section does not exist').to.exist
        expect(results, 'No results whatsoever for this section').to.be.not.empty
    })

    it('Testing home page results for update titles', async () => {
        const results = await wrapper.getViewMoreItems(source, 'update', {}, 1)
    ...
    it('Testing home page results for today titles', async () => {
        const results = await wrapper.getViewMoreItems(source, 'today', {}, 1)
    ...
    it('Testing Notifications', async () => {
        const updates = await wrapper.filterUpdatedManga(source, new Date('2022-07-07'), [mangaId])
