...
export const parseMangaDetails = ($: CheerioStatic, mangaId: string): Manga => {
    ...
    const rawStatus: string = $('span:contains(Status)', contentSection).parent().text().trim()
...
export const parseChapterDetails = (data: any, mangaId: string, chapterId: string): ChapterDetails => {
    ...
    console.log(JSON.stringify(imageMatches))
...
export const parseSearch = ($: CheerioStatic, cheerio: any): MangaTile[] => {
    ...
    if (directMatch.toLocaleLowerCase() == 'cover') {
        ...
        console.log('listing')
        console.log($('div.list-comic').toArray().length)
        console.log('items')
        console.log($('div.item', $('div.list-comic').first()).toArray().length)

        for (const item of $('div.item', $('div.list-comic').first()).toArray()) {
            const rawHTML: string = $(item).attr('title')?.trim() ?? ''
            const cheerioData = cheerio.load(rawHTML)

            const title: string = cheerioData('p').first().text().trim()
            const id: string = $('a', item).attr('href')?.split('/').pop()?.split('?').shift() ?? ''
            let image: string = $('img', item).attr('src') ?? 'https://i.imgur.com/GYUxEX8.png'
            image = image.startsWith('/') ? RCO_DOMAIN + image : image

            const subtitle: string = cheerioData('strong:contains(Status)').parent().text().trim()()
            console.log('subtitle')
            console.log(subtitle)

            const subtitle2: string = cheerioData('strong:contains(Status)').text().trim()()
            console.log('subtitle2')
            console.log(subtitle2)

            const subtitle3: string = cheerioData('strong:contains(Status)').children().remove().end().text().trim()()
            console.log('subtitle3')
            console.log(subtitle3)

            const subtitle4: string = cheerioData('strong:contains(Status)').parent().children().remove().end().text().trim()()
