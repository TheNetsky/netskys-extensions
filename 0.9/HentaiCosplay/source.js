...
var _Sources = (() => {
  ...
  var HentaiCosplay_exports = {};
  __export(HentaiCosplay_exports, {
    HentaiCosplay: () => HentaiCosplay,
    HentaiCosplayInfo: () => HentaiCosplayInfo
  });
  ...
  // src/HentaiCosplay/HentaiCosplayParser.ts
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const title = decode($2("span#title > h2").text().trim());
    const image = $2("img", $2("div.icon-overlay")).first().attr("src") ?? "";
    const description = `Gallery: ${title}`;
    const arrayTags = [];
    for (const tag of $2("span", "p#detail_tag").toArray()) {
      const id = $2("a", tag).attr("href")?.trim().replace(/\/$/, "").split("/").pop();
      if (!id) continue;
      arrayTags.push({ id, label: id });
    }
  ...
  var parseChapterDetails = ($2, mangaId) => {
    const pages = [];
    for (const img of $2("amp-story-grid-layer > amp-img").toArray()) {
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    const sections = [
      {
        ...
        selector: $2("ul#image-list").get(2)
      },
      {
        sectionID: App.createHomeSection({
          id: "new",
    ...
    for (const section of sections) {
      const itemArray = [];
      for (const item of $2("div.image-list-item", section.selector).toArray()) {
        const id = $2("a", item).attr("href")?.replace("/image", "").replace("/", "");
        const image = $2("img", item).first().attr("src") ?? "";
        const title = $2("p.image-list-item-title > a", item).text().trim();
        const subtitle = $2("p.image-list-item-regist-date > span", item).text().trim();
  ...
  var parseViewMore = ($2) => {
    const manga = [];
    const collectedIds = [];
    for (const item of $2("div.image-list-item", "ul#image-list").toArray()) {
      const id = $2("a", item).attr("href")?.replace("/image", "").replace("/", "");
      const image = $2("img", item).first().attr("src") ?? "";
      const title = $2("p.image-list-item-title > a", item).text().trim();
      const subtitle = $2("p.image-list-item-regist-date > span", item).text().trim();
  ...
  var parseTags = ($2) => {
    const arrayTags = [];
    for (const tag of $2("#tags li").toArray()) {
      const id = $2("a", tag).attr("href")?.trim().replace(/\/$/, "").split("/").pop();
      if (!id) continue;
      arrayTags.push({ id, label: id });
    }
  ...
  var isLastPage = ($2) => {
    let isLast = false;
    const lastPage = $2("div.wp-pagenavi > a.last").attr("href");
  ...
  var HentaiCosplay = class _HentaiCosplay {
    constructor() {
    ...
    getMangaShareUrl(mangaId) {
      return `${HC_DOMAIN}/image/${mangaId}`;
    }
    ...
    async getChapterDetails(mangaId) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      return parseChapterDetails($2, mangaId);
    }
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        request = App.createRequest({
          url: `${HC_DOMAIN}/search/tag/${query?.includedTags?.map((x) => x.id)[0]}/page/${page}`,
          method: "GET"
        });
      }
      const response = await this.requestManager.schedule(request, 1);
...
Please go to the homepage of <${_HentaiCosplay.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(HentaiCosplay_exports);
})();
