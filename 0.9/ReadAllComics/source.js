...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const title = decode($2("h1").text()?.trim() ?? "");
    const image = $2("img", ".description-archive").first().attr("src") ?? "";
    const description = $2("div.b > strong", ".description-archive").text().trim();
    const publisher = $2("strong", "p:contains(Publisher:)").last().text().trim() ?? "";
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    const chapterListElement = $2("li", "ul.list-story").toArray();
    let i = chapterListElement.length;
    for (const chapter of chapterListElement) {
      const id = $2("a", chapter).attr("href")?.replace(/\/$/, "").split("/").pop() ?? "";
      if (!id) continue;
      const title = $2("a", chapter).text().trim() ?? "";
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const img of $2("img", "center > div:nth-child(2)").toArray()) {
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    const newCompletedSection_Array = [];
    for (const item of $2("div", "div#post-area").toArray()) {
      const classAttribute = $2(item).attr("class") ?? "";
      const idRegex = classAttribute?.match(/category-([^ ]+)/);
      let id = "";
      if (idRegex && idRegex[1]) id = idRegex[1];
      const title = $2("h2", item).text().trim() ?? "";
      const subtitle = $2("span", item).first().text().trim();
  ...
  var parseSearchResults = ($2) => {
    const comics = [];
    const collectedIds = [];
    for (const item of $2("li", "ul.list-story").toArray()) {
      const id = $2("a", item).attr("href")?.replace(/\/$/, "").split("/").pop() ?? "";
      const title = $2(item).text().trim() ?? "";
      if (!id || !title || collectedIds.includes(id)) continue;
      comics.push(App.createPartialSourceManga({
        image: "",
        // There is no image in search results
        title: decode(title),
        mangaId: id
      }));
    }
  ...
  var isLastPage = ($2) => {
    let isLast = false;
    const lastPage = $2("a:contains(Next)") ?? "";
    if (!lastPage) isLast = true;
    return isLast;
  };

  // src/ReadAllComics/ReadAllComics.ts
  var RC_DOMAIN = "https://readallcomics.com";
  ...
  var ReadAllComics = class _ReadAllComics {
    constructor() {
    ...
    getMangaShareUrl(mangaId) {
      return `${RC_DOMAIN}/category/${mangaId}`;
    }
    ...
    async getSearchResults(query, metadata) {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const $2 = load(response.data);
      const manga = parseSearchResults($2);
      return App.createPagedResults({
        results: manga,
        metadata
      });
...
Please go to the homepage of <${_ReadAllComics.name}> and press the cloud icon.`);
      }
    }
