...
var _Sources = (() => {
  ...
  var MangaHasu_exports = {};
  __export(MangaHasu_exports, {
    MangaHasu: () => MangaHasu,
    MangaHasuInfo: () => MangaHasuInfo
  });
  ...
  // src/MangaHasu/MangaHasuParser.ts
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push(decode($2("div.info-title > h1").text()?.trim() ?? ""));
    const altTitles = $2("div.info-title > h3").text().trim().split(";");
    for (const title of altTitles) {
      titles.push(decode(title.trim()));
    }
    const image = $2("img", "div.col-md-4.col-sm-4.info-img").attr("src") ?? "";
    const description = decode($2("div.content-info > div").first().text().trim() ?? "");
    const infoSections = $2(".info-c").first();
    const author = $2($2(".info", infoSections).get(0)).text().trim();
    const artist = $2($2(".info", infoSections).get(1)).text().trim();
    const arrayTags = [];
    for (const tag of $2("a", $2(".info", infoSections).get(3)).toArray()) {
      const label = $2(tag).text().trim();
      const id = encodeURI(idCleaner($2(tag).attr("href")?.split("/").pop()?.trim() ?? ""));
      if (!id || !label) continue;
      arrayTags.push({ id, label });
    }
    const tagSections = [App.createTagSection({ id: "0", label: "genres", tags: arrayTags.map((x) => App.createTag(x)) })];
    const rawStatus = $2($2(".info", infoSections).get(4)).text().trim();
    let status = "ONGOING";
  ...
  var parseChapters = ($2, mangaId, baseURL) => {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2("tr", "div.list-chapter").toArray()) {
      const title = decode($2("td.name > a", chapter).children().remove().end().text().trim());
      const chapterId = $2("a", chapter).attr("href")?.replace(baseURL + "/", "") ?? "";
      if (!chapterId || !title) continue;
      const date = new Date($2("td.date-updated", chapter).text().trim());
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const img of $2("img", "div.img").toArray()) {
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("div.div_item", "div.pinked-content").toArray()) {
    ...
    for (const manga of $2("div.div_item", "div.col-xs-12.wrapper_content").toArray()) {
      const image = $2("img", manga).first().attr("src") ?? "";
      const title = $2("img", manga).first().attr("alt") ?? "";
      const id = $2("a", manga).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const subtitle = $2("a.name-chapter > span", manga).text().trim() ?? "";
    ...
    for (const manga of $2("li", "div.col-xs-12.mgtop10.mvtd").toArray()) {
      const image = $2("img", manga).first().attr("src") ?? "";
      const title = $2("img", manga).first().attr("alt") ?? "";
      const id = $2("a", manga).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
  ...
  var parseViewMore = ($2) => {
    const manga = [];
    const collectedIds = [];
    for (const obj of $2("div.div_item", "div.st_content > ul.list_manga").toArray()) {
  ...
  var parseTags = ($2) => {
    const arrayTags = [];
    for (const tag of $2("li", "li.dropdown.genres").toArray()) {
      const label = $2(tag).text().trim() ?? "";
      const id = encodeURI(idCleaner($2("a", tag).attr("href")?.split("/").pop()?.trim() ?? ""));
      if (!id || !label) continue;
      arrayTags.push({ id, label });
    }
    const tagSections = [App.createTagSection({ id: "0", label: "genres", tags: arrayTags.map((x) => App.createTag(x)) })];
  ...
  var isLastPage = ($2) => {
    let isLast = true;
    const hasNext = Boolean($2("a:contains(Last)", "div.pagination-ct").first());
  ...
  var MangaHasu = class _MangaHasu {
    constructor() {
    ...
    async getChapters(mangaId) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      return parseChapters($2, mangaId, MH_DOMAIN);
    }
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        request = App.createRequest({
          url: `${MH_DOMAIN}/${query?.includedTags?.map((x) => x.id)[0]}?page=${page}`,
          method: "GET"
        });
      }
      const response = await this.requestManager.schedule(request, 1);
...
Please go to the homepage of <${_MangaHasu.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(MangaHasu_exports);
})();
