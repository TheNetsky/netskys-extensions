...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    ...
    const titles = [(0, import_html_entities.decode)($("img", "figure.cover").attr("alt") ?? ""?.trim() ?? "")];
    const author = $("span", "div.author").last().text().trim();
    const description = (0, import_html_entities.decode)($("p.description").text().trim() ?? "");
    const arrayTags = [];
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (const chapter of $("li", "ul.chapter-list").toArray()) {
      const title = (0, import_html_entities.decode)($("strong.chapter-title", chapter).text().trim());
  ...
  var parseChapterDetails = async ($, source, mangaId, chapterId, cheerio, requestManager) => {
    const pages = [];
    const scriptRegex = (0, import_html_entities.decode)($.html()).match(/loaadchppa\('([\w\d]+)'\)/);
    let loadMoreId = "";
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const manga of $("li.novel-item", $("h3:contains(Our Translation)").parent().parent()).toArray()) {
      ...
      const subtitle = $("p.lastChapter", manga).text().trim() ?? "";
    ...
    for (const manga of $(".holder.boxsizing").toArray()) {
      ...
      const title = $("img", manga).first().attr("alt") ?? "";
      const id = $("a", manga).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const subtitle = $("h5.chapternumber", manga).text().replace("update", "").trim() ?? "";
      if (!id || !title) continue;
  ...
  var parseViewMore = ($) => {
    ...
    for (const item of $(".holder.boxsizing").toArray()) {
      ...
      const title = $("img", item).first().attr("alt") ?? "";
      const id = $("a", item).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const subtitle = $("h5.chapternumber", item).text().replace("update", "").trim() ?? "";
