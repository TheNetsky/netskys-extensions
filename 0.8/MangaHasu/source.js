...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($("div.info-title > h1").text()?.trim() ?? ""));
    ...
    const description = (0, import_html_entities.decode)($("div.content-info > div").first().text().trim() ?? "");
  ...
  var parseChapters = ($, mangaId, baseURL) => {
    ...
    for (const chapter of $("tr", "div.list-chapter").toArray()) {
      const title = (0, import_html_entities.decode)($("td.name > a", chapter).children().remove().end().text().trim());
