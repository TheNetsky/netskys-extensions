...
var _Sources = (() => {
  ...
  var McReader_exports = {};
  __export(McReader_exports, {
    McReader: () => McReader,
    McReaderInfo: () => McReaderInfo
  });
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push(decode($2("img", "div.fixed-img").attr("alt")?.trim() ?? ""));
    ...
    const description = decode($2("p.description").text().split("The Summary is\n\n")[1]?.trim() ?? "");
    const arrayTags = [];
  ...
  var parseChapters = ($2, mangaId) => {
    ...
    for (const chapter of $2("li", "ul.chapter-list").toArray()) {
      const title = decode($2("strong.chapter-title", chapter).text().trim());
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("li.novel-item", "ul.novel-list").toArray()) {
      const image = $2("img", manga).first().attr("data-src") ?? "";
      const title = $2("img", manga).first().attr("alt") ?? "";
      const id = $2("a", manga).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const subtitle = $2("span", $2("div.novel-stats", manga)).first().text().replace("update", "").trim() ?? "";
      if (!id || !title) continue;
      updateSection_Array.push(App.createPartialSourceManga({
        image,
        title: decode(title),
        mangaId: id,
        subtitle: decode(subtitle + " ago")
      }));
    }
  ...
  var McReader = class _McReader {
    ...
    getMangaShareUrl(mangaId) {
      return `${MCR_DOMAIN}/manga/${mangaId}`;
    }
...
Please go to the homepage of <${_McReader.name}> and press the cloud icon.`);
      }
    }
