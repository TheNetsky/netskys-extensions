...
var _Sources = (() => {
  ...
  // src/ComicOnlineFree/ComicOnlineFree.ts
  var ComicOnlineFree_exports = {};
  __export(ComicOnlineFree_exports, {
    ComicOnlineFree: () => ComicOnlineFree,
    ComicOnlineFreeInfo: () => ComicOnlineFreeInfo
  });
  ...
  // src/ComicOnlineFree/ComicOnlineFreeParser.ts
  var import_types2 = __toESM(require_lib());
  var import_html_entities = __toESM(require_lib2());
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($2("td:contains(Name:)").first().next().text().trim()));
    titles.push((0, import_html_entities.decode)($2("td:contains(Alternate Name:)").next().text().trim()));
    let image = $2("img", "div.manga-image").attr("src") ?? "";
    image = image.startsWith("/") ? "https:" + image : image;
    const author = $2("td:contains(Author:)").next().text().trim();
    const description = (0, import_html_entities.decode)($2("p.pdesc").text().trim());
    const arrayTags = [];
    for (const tag of $2("a", $2("td:contains(Genre)").next()).toArray()) {
    ...
    const rawStatus = $2("td:contains(Status:)").next().text().trim();
    let status = "ONGOING";
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2("li", "ul.basic-list").toArray()) {
      const title = (0, import_html_entities.decode)($2("a.ch-name", chapter).text().trim());
      const chapterId = $2("a", chapter).attr("href")?.split("/").pop() ?? "";
      if (!chapterId) continue;
      const date = new Date($2("span", chapter).text());
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const img of $2("img", "div.chapter-container").toArray()) {
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("li.list-top-movie-item", "div.right-box-content").toArray()) {
      let image = $2("div.list-top-movie-item-thumb", manga).attr("style") ?? "";
      const urlRegex = image.match(/url\(['"]?(.*?)['"]?\)/);
      if (urlRegex && urlRegex[1]) image = urlRegex[1];
      const title = $2("span.list-top-movie-item-vn", manga).text().trim() ?? "";
      const id = $2("a", manga).attr("href")?.split("/").pop()?.trim();
      let subtitle = $2("a.chapter", manga).text() ?? "";
    ...
    for (const manga of $2("li.manga-box", "ul.home-list").toArray()) {
      const image = $2("img", manga).attr("src") ?? "";
      const title = $2("img", manga).attr("alt")?.trim() ?? "";
      const id = $2("a", manga).attr("href")?.split("/").pop()?.trim();
      let subtitle = $2("div.detail > a", manga).text().trim();
      subtitle = subtitle.substring(subtitle.indexOf("#"))?.trim();
      if (!id || !title) continue;
  ...
  var parseViewMore = ($2) => {
    const manga = [];
    const collectedIds = [];
    for (const obj of $2("div.manga-box", "div.container").toArray()) {
      const image = $2("img", obj).attr("src") ?? "";
      const title = $2("img", obj).attr("alt")?.trim() ?? "";
      const id = $2("a", obj).attr("href")?.split("/").pop()?.trim();
      let subtitle = $2("div.detail > a", obj).first().text().trim();
      subtitle = subtitle.substring(subtitle.indexOf("#"))?.trim();
  ...
  var parseTags = ($2) => {
    const arrayTags = [];
    for (const tag of $2("li", "ul.search-checks").toArray()) {
      const label = $2(tag).text().trim() ?? "";
      const id = $2(tag).text().trim().replace(/\s/g, "+") ?? "";
      if (!id || !label) continue;
      arrayTags.push({ id, label });
    }
    const tagSections = [App.createTagSection({ id: "0", label: "genres", tags: arrayTags.map((x) => App.createTag(x)) })];
  ...
  var parseSearch = ($2) => {
    const mangas = [];
    for (const obj of $2("div.manga-box", "div.result-left").toArray()) {
      const id = $2("a", obj).attr("href")?.split("/").pop()?.trim();
      const image = $2("img", obj).attr("src") ?? "";
      const title = $2("img", obj).attr("alt")?.trim() ?? "";
      const subtitle = $2("div.detail", obj).first().text().trim();
      if (!id || !title) continue;
  ...
  var isLastPage = ($2) => {
    let isLast = false;
    const items = [];
    for (const page of $2("a", "div.general-nav").toArray()) {
      items.push($2(page).text().trim().toLowerCase());
    }
    if (!items.includes("next")) isLast = true;
    return isLast;
  };

  // src/ComicOnlineFree/ComicOnlineFreeHelper.ts
  ...
  // src/ComicOnlineFree/ComicOnlineFree.ts
  var COF_DOMAIN = "https://readcomic.net";
  var ComicOnlineFreeInfo = {
    version: "1.1.9",
  ...
  var ComicOnlineFree = class _ComicOnlineFree {
    constructor() {
    ...
    getMangaShareUrl(mangaId) {
      return `${COF_DOMAIN}/comic/${mangaId}`;
    }
    ...
    async getSearchResults(query, metadata) {
      const page = metadata?.page ?? 1;
      const url = new URLBuilder(COF_DOMAIN).addPathComponent("advanced-search").addQueryParameter("key", encodeURI(query?.title || "")).addQueryParameter("page", page).addQueryParameter("wg", query.includedTags?.map((x) => x.id).join("%2C")).buildUrl();
      const request = App.createRequest({
        url,
        method: "GET"
      });
...
Please go to the homepage of <${_ComicOnlineFree.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(ComicOnlineFree_exports);
})();
