...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($("td:contains(Name:)").first().next().text().trim()));
    titles.push((0, import_html_entities.decode)($("td:contains(Alternate Name:)").next().text().trim()));
    let image = $("img", "div.manga-image").attr("src") ?? "";
    image = image.startsWith("/") ? "https:" + image : image;
    const author = $("td:contains(Author:)").next().text().trim();
    const description = (0, import_html_entities.decode)($("p.pdesc").text().trim());
    const arrayTags = [];
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (const chapter of $("li", "ul.basic-list").toArray()) {
      const title = (0, import_html_entities.decode)($("a.ch-name", chapter).text().trim());
