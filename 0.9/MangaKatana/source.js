...
var _Sources = (() => {
  ...
  __export(MangaKatana_exports, {
    ...
    MangaKatanaInfo: () => MangaKatanaInfo
  });
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const titles = [];
    titles.push(decode($2("h1.heading").first().text().trim()));
    const altTitles = $2("div.alt_name").text().split(";");
    for (const title of altTitles) {
      titles.push(decode(title.trim()));
    }
    const image = $2("div.media div.cover img").attr("src") ?? "";
    const author = $2(".author").text().trim();
    const description = decode($2(".summary > p").text().trim());
    const arrayTags = [];
    for (const tag of $2(".genres > a").toArray()) {
      const label = $2(tag).text().trim();
      const id = $2(tag).attr("href")?.split("genre/")[1] ?? "";
      if (!id || !label) continue;
      arrayTags.push({ id, label });
    }
    const tagSections = [App.createTagSection({ id: "0", label: "genres", tags: arrayTags.map((x) => App.createTag(x)) })];
    const rawStatus = $2(".value.status").text().trim();
    let status = "ONGOING";
  ...
  var parseChapters = ($2, mangaId) => {
    const chapters = [];
    for (const elem of $2("tr:has(.chapter)").toArray()) {
      const title = $2("a", elem).text();
      const date = new Date($2(".update_time", elem).text() ?? "");
      const chapterId = $2("a", elem).attr("href")?.split("/").pop();
  ...
  var parseChapterDetails = (data2, mangaId, chapterId) => {
    const pages = [];
    let images = [];
    const imageArrayYtaw = data2.match(/var ytaw=\[(.*?)\]/);
    let imagesYtaw = [];
    if (imageArrayYtaw && imageArrayYtaw[1]) imagesYtaw = imageArrayYtaw[1]?.replace(/'/g, "")?.split(",");
    const imageArrayThzq = data2.match(/var thzq=\[(.*?)\]/);
    let imagesThzq = [];
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const manga of $2("div.item", "div#hot_update").toArray()) {
      const title = $2(".title", manga).text().trim();
    ...
    const hotManga = [];
    for (const manga of $2("div.item", "div#hot_book").toArray()) {
      const title = $2(".title", manga).text().trim();
      const id = $2("a", manga).attr("href")?.split("/").pop() ?? "";
      const image = $2("img", manga).attr("data-src") ?? "";
      const subtitle = $2(".chapter", manga).first().text().trim();
      if (!id || !title) continue;
    ...
    const latestManga = [];
    for (const manga of $2("div.item", "div#book_list").toArray()) {
      const title = $2(".title", manga).text().trim();
      const id = $2("a", manga).attr("href")?.split("/").pop() ?? "";
      const image = $2("img", manga).first().attr("src") ?? "";
      const subtitle = $2(".chapter", manga).first().text().trim();
  ...
  var parseSearch = ($2) => {
    ...
    if ($2('meta[property="og:url"]').attr("content")?.includes("/manga/")) {
      ...
      const id = $2("meta[property$=url]").attr("content")?.split("/")?.pop() ?? "";
      const image = $2("div.media div.cover img").attr("src") ?? "";
  ...
  var parseViewMore = ($2) => {
    const manga = [];
    for (const p of $2("div.item", "div#book_list").toArray()) {
      const title = $2(".title a", p).text().trim();
      const id = $2("a", p).attr("href")?.split("/").pop() ?? "";
      const image = $2("img", p).attr("src") ?? "";
      const subtitle = $2(".chapter", p).first().text().trim();
      if (!id || !title) continue;
  ...
  var isLastPage = ($2) => {
    let isLast = true;
    const hasNext = Boolean($2("a.next.page-numbers", "ul.uk-pagination").text());
