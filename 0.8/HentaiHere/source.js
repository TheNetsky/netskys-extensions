...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($("h4 > a").first()[0]?.firstChild.data?.trim() ?? ""));
    const image = $("img", "div#cover").attr("src") ?? "";
    const artist = $("span:contains(Artist:)").next().text().trim();
    const description = (0, import_html_entities.decode)($("span:contains(Brief Summary:)").parent().text().replace($("span:contains(Brief Summary:)").text(), "").trim());
    const arrayTags = [];
  ...
  var parseChapters = ($) => {
    ...
    for (const chapter of $("li.sub-chp", "ul.arf-list").toArray()) {
      const title = (0, import_html_entities.decode)($("span.pull-left", chapter).text().replace($("span.pull-left i.text-muted", chapter).text(), "").trim());
  ...
  var parseChapterDetails = (data, mangaId, chapterId) => {
    const pages = [];
    let obj = /var rff_imageList = (.*);/.exec(data)?.[1] ?? "";
  ...
  var parseSearch = ($) => {
    ...
    for (const obj of $("div.item", "div.row.row-sm").toArray()) {
      const id = $("a", obj).attr("href")?.split("/").pop()?.trim();
      const image = $("img", obj).attr("src") ?? "";
      const title = (0, import_html_entities.decode)(String($("img", obj).attr("alt")?.trim()));
