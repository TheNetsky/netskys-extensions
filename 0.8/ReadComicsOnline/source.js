...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($, mangaId) => {
    const titles = [];
    titles.push((0, import_html_entities.decode)($("h2.listmanga-header").first().text().trim()));
    let image = $("img", "div.boxed").attr("src") ?? "";
    if (image.startsWith("/")) image = "https:" + image;
    const author = $("dd", $("dt:contains(Type)").parent()).text().trim() ?? "";
    const description = (0, import_html_entities.decode)($("p", "div.manga.well").text().trim() ?? "");
    const arrayTags = [];
