...
var source = (() => {
  ...
  // src/Readm/Readm.ts
  var Readm_exports = {};
  __export(Readm_exports, {
    Readm: () => Readm,
    ReadmInfo: () => ReadmInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/Readm/ReadmParser.ts
  var import_types = __toESM(require_lib());
  var entities = require_lib2();
  var RM_DOMAIN = "https://readm.org";
  var parseMangaDetails = ($, mangaId) => {
    ...
    const description = decodeHTMLEntity($("p", "div.series-summary-wrapper").text().trim() ?? "No description available");
    const arrayTags = [];
  ...
  var parseChapters = ($, mangaId) => {
    ...
    for (const chapter of $("div.season_start").toArray()) {
      ...
      const rawChapterId = $("a", chapter).attr("href") ?? "";
      ...
      const date = parseDate($("td.episode-date", chapter)?.text() ?? "");
  ...
  var parseHomeSections = ($, sectionCallback) => {
    ...
    for (const manga of $("ul#latest_trailers li").toArray()) {
      ...
      const parseImage = getImageSrc($("img", manga));
      const image = parseImage ? RM_DOMAIN + parseImage : "";
      const subtitle = $("small", manga).first().text().trim() ?? "";
      if (!id || !title) continue;
  ...
  var parseViewMore = ($, homepageSectionId) => {
    const manga = [];
  ...
  var isLastPage = ($) => {
    let isLast = true;
    const hasNext = Boolean($("a:contains(\xBB)", "div.ui.pagination.menu")[0]);
  ...
  // src/Readm/Readm.ts
  var RM_DOMAIN2 = "https://readm.today";
  var ReadmInfo = {
    version: "2.1.7",
    name: "Readm",
    icon: "icon.png",
  ...
  var Readm = class _Readm {
    ...
    getMangaShareUrl(mangaId) {
      return `${RM_DOMAIN2}/manga/${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${RM_DOMAIN2}/manga/${mangaId}`,
        method: "GET",
        headers: {
    ...
    async getChapterDetails(mangaId, chapterId) {
      const request = App.createRequest({
        url: `${RM_DOMAIN2}/manga/${mangaId}/${chapterId}/all-pages`,
        method: "GET"
      });
      const response = await this.requestManager.schedule(request, 1);
      this.CloudFlareError(response.status);
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        for (const m of data.manga) {
          ...
          const image = RM_DOMAIN2 + m.image;
          const title = m.title;
          if (!id || !title) continue;
          manga.push(App.createPartialSourceManga({
            image,
        ...
        const request = App.createRequest({
          url: `${RM_DOMAIN2}/category/${query?.includedTags?.map((x) => x.id)[0]}/watch/${page}`,
          method: "GET"
        });
        const response = await this.requestManager.schedule(request, 1);
        this.CloudFlareError(response.status);
...
Please go to the homepage of <${_Readm.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(Readm_exports);
})();
