...
var _Sources = (() => {
  ...
  var Mitaku_exports = {};
  __export(Mitaku_exports, {
    Mitaku: () => Mitaku,
    MitakuInfo: () => MitakuInfo
  });
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const title = decode($2("h1.cm-entry-title").first().text().trim());
    const artist = decode($2("p:contains(Cosplayer:)").text().trim().replace("Cosplayer:", "").trim());
...
Images: ${images.length}`;
    const arrayTags = [];
    for (const tag of $2("a", "span.tag-links").toArray()) {
      const id = $2(tag).attr("href")?.split("/tag/")[1]?.replace(/\//g, "");
  ...
  var parseImages = ($2) => {
    const images = [];
    for (const img of $2("a", "div.msacwl-slider-wrap").toArray()) {
      let image = $2(img).attr("src");
      if (!image) image = $2(img).attr("data-mfp-src");
      if (!image) image = $2(img).attr("data-lazy");
      if (!image) continue;
      images.push(image);
    }
    return images;
  };
  var getImageSrc = (imageObj) => {
    let image;
  ...
  var isLastPage = ($2) => {
    let isLast = true;
    const hasNext = Boolean($2("a.last", "div.wp-pagenavi").first());
  ...
  var Mitaku = class _Mitaku {
      ...
      for (const section of sections) {
        ...
        promises.push(
          this.requestManager.schedule(section.request, 1).then((response) => {
            this.CloudFlareError(response.status);
            const $2 = load(response.data);
            const items = parseHomeSections($2);
    ...
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      this.CloudFlareError(response.status);
      const $2 = load(response.data);
      const manga = parseHomeSections($2);
    ...
    async getSearchTags() {
      ...
      const response = await this.requestManager.schedule(request, 1);
      const tagJSON = JSON.parse(response.data);
      const arrayTags = [];
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        request = App.createRequest({
          url: `${MT_DOMAIN}/tag/${query?.includedTags?.map((x) => x.id)[0]}/page/${page}`,
          method: "GET"
        });
      }
      const response = await this.requestManager.schedule(request, 1);
...
Please go to the homepage of <${_Mitaku.name}> and press the cloud icon.`);
      }
    }
  ...
  return __toCommonJS(Mitaku_exports);
})();
