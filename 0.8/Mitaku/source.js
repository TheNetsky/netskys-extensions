...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    ...
    const title = decode($2(".cm-entry-title").first().text().trim());
...
Images: ${images.length}`;
    ...
    for (const obj of $2("div.genres-content a").toArray()) {
      const id = idCleaner($2(obj).attr("href") ?? "");
      const title2 = $2(obj).text().trim();
      if (!title2 || !id) continue;
      arrayTags.push({ label: title2, id });
    }
  ...
  // src/Mitaku/Mitaku.ts
  var MT_DOMAIN2 = "https://mitaku.net";
  var MitakuInfo = {
    version: "1.0.4",
    name: "Mitaku",
    icon: "icon.png",
  ...
  var Mitaku = class _Mitaku {
    ...
    getMangaShareUrl(mangaId) {
      return `${MT_DOMAIN2}/?p=${mangaId}`;
    }
    async getMangaDetails(mangaId) {
      const request = App.createRequest({
        url: `${MT_DOMAIN2}/?p=${mangaId}`,
        method: "GET"
      });
      ...
      const sections = [
        {
          request: App.createRequest({
            url: `${MT_DOMAIN2}/category/ero-cosplay`,
            method: "GET"
          }),
        ...
        {
          request: App.createRequest({
            url: `${MT_DOMAIN2}/category/sexy-set`,
            method: "GET"
          }),
          sectionID: App.createHomeSection({
            id: "sexy-set",
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        request = App.createRequest({
          url: `${MT_DOMAIN2}/page/${page}?s=${encodeURI(query.title)}`,
          method: "GET"
        });
      } else {
        request = App.createRequest({
          url: `${MT_DOMAIN2}/tag/${query?.includedTags?.map((x) => x.id)[0]}/page/${page}`,
          method: "GET"
        });
      }
      const response = await this.requestManager.schedule(request, 1);
